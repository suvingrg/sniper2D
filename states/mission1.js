...
sniper2D.mission1 = function (game) {
...
sniper2D.mission1.prototype = {
    ...
    this.gameControls = this.add.bitmapText(550, 600, 'gunplay', 'GAME CONTROLS', 100);
    this.zoomIn = this.add.bitmapText(10, 810, 'gunplay', 'ZOOM IN', 48);
		this.we = this.controls.create(210, 800, 'we');
    this.zoomOut = this.add.bitmapText(390, 810, 'gunplay', 'ZOOM OUT', 48);
		this.spacebar = this.controls.create(775, 800, 'spacebar');
    this.fire = this.add.bitmapText(700, 740, 'gunplay', 'Press SPACEBAR to fire', 48);
		this.dpad = this.controls.create(1550, 770, 'dpad');
    this.moveCrosshair = this.add.bitmapText(1490, 720, 'gunplay', 'Move Crosshair using Cursors');

		// this.controls.anchor.setTo(0.5);
		this.controls.scale.set(0.7);
    this.controls.add(this.gameControls);
    this.controls.add(this.zoomIn);
    this.controls.add(this.zoomOut);
    this.controls.add(this.fire);
    this.controls.add(this.moveCrosshair);

    this.startText = this.add.bitmapText(this.camera.width * 0.5, this.camera.height * 0.5, 'destroy', 'TAP TO START', 48);
    this.startText.anchor.setTo(0.5);
    this.startText.alpha = 0;
    this.add.tween(this.startText).to({alpha: 1}, 10000, 'Linear', true);
    this.startText.inputEnabled = true;
    this.startText.events.onInputDown.add(this.mainCreate, this);
  ...
  render: function (game) {
    ...
    if (this.debugInfo) {
      game.debug.text(this.time.fps || '--', 2, 20, "#0000ff", '32px Verdana');
  ...
  mainCreate: function () {
    ...
    this.missionText = this.add.bitmapText(this.camera.width * 0.5, 20, 'destroy', 'MISSION 1', 18);
    this.missionText.anchor.setTo(0.5);
